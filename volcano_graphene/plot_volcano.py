...
def filter_and_plot_data(file_path, output_dir=None):
    ...
    - Filters rows where delta_E_HOO-HO(eV) is between 2.0 and 4.0.
    - Builds step energies from delta_E1, delta_E2, and delta_E3_HOO.
    ...
    try:
        ...
        # --- Filtering ---
        filter_column_name = "delta_E_HOO-HO(eV)"
        if filter_column_name not in df.columns:
            print(f"Error: Required filter column '{filter_column_name}' not found.")
            return
        ...
        # --- Build ΔG1-4 from HOO-only cumulative adsorption energies ---
        dg_cols = ["delta_E1(eV)", "delta_E2(eV)", "delta_E3_HOO(eV)"]
        missing_energy_cols = [col for col in dg_cols if col not in filtered_df.columns]
        if missing_energy_cols:
            print(f"Error: Required energy columns not found: {missing_energy_cols}")
            return
        filtered_df[dg_cols] = filtered_df[dg_cols].apply(pd.to_numeric, errors="coerce")
        filtered_df.dropna(subset=dg_cols, inplace=True)
        if filtered_df.empty:
            print("No valid rows after parsing HOO energy columns.")
            return
